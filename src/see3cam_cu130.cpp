 ...
 * @return true/false
 */
bool See3CAM_CU130::setSmileDetection(bool enableSmileDetect, bool embedData, uint thresholdValue, bool smileTrigger){
    // hid validation
    if(uvccamera::hid_fd < 0)
    {
        return false;
    ...
    if(smileTrigger)
        g_out_packet_buf[6] = ENABLE_SMILE_TRIGGER_CU130; /* enable smile trigger  */
    else
        g_out_packet_buf[6] = DISABLE_SMILE_TRIGGER_CU130; /* disable smile trigger */
